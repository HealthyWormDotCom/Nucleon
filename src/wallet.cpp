...
// Nucleon: optional setting to unlock wallet for block minting only;
...
// Nucleon: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // Nucleon: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // Nucleon: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    else
                    {
                        ...
                        if (!GetBoolArg("-avatar")) // Nucleon: not avatar mode
                        {
...
// Nucleon: create coin stake transaction
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64 nSearchInterval, CTransaction& txNew)
{
    ...
    CBigNum bnTargetPeNucleonDay;
    bnTargetPeNucleonDay.SetCompact(nBits);
...
// Nucleon: check 'spent' consistency between wallet and txindex
// Nucleon: fix wallet spent state according to txindex
...
// Nucleon: disable transaction (only for coinstake)
