...
const string strMessageMagic = "Nucleon Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // Nucleon: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
bool CTransaction::CheckTransaction() const
{
    ...
    for (int i = 0; i < vout.size(); i++)
    {
        ...
        // Nucleon: enforce minimum output amount
                        ...
                        bool* pfMissingInputs)
{
    ...
    // Nucleon: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// Nucleon: find block wanted by given orphan block
...
{
    int64 nSubsidy = CENT;
	if (nBits == 469827583) { nSubsidy = 10000 * COIN; }
	return nSubsidy;
}

int64 GetProofOfStakeReward(int64 nCoinAge, int nHeight)
{
	int64 nSixth = nHeight % 10;
	int64 nFifth = (nCoinAge % 10) * 10;
	int64 nFourth = ((nCoinAge * 2) % 10) * 100;
	int64 nThird = ((nHeight * 2) % 10) * 1000;
	int64 nSecond = ((nCoinAge * nHeight) % 10) * 10000;
	int64 nFirst = ((nCoinAge + nHeight) % 10) * 100000;
	int64 nSubsidy = nFirst + nSecond + nThird + nFourth + nFifth + nSixth;
	if (nSubsidy < 10000) { nSubsidy = CENT; }
	return nSubsidy;
}
...
// Nucleon: find last block index up to pindex
...
unsigned int static GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    if (pindexPrevPrev->pprev == NULL)
		...
		if (!fProofOfStake && pindexLast->nHeight > 1 && pindexLast->nHeight < 100)
	        return bnProofOfWorkLimit.GetCompact();

    int64 nActualSpacing = pindexPrev->GetBlockTime() - pindexPrevPrev->GetBlockTime();

    // Nucleon: target change every block
    // Nucleon: retarget with exponential moving toward target spacing
    CBigNum bnNew;
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    // Nucleon: should not enter safe mode for longer invalid chain
}
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Nucleon: check transaction timestamp
        ...
        if (IsCoinStake())
        {
            // Nucleon: coin stake tx earns reward instead of paying fee
            uint64 nCoinAge;
        ...
        else
        {
            ...
            // Nucleon: enforce transaction fees for every block
            if (nTxFee < GetMinFee())
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // Nucleon: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // Nucleon: track money supply and mint amount info
    ...
    // Nucleon: fees are not collected by miners as in bitcoin
    // Nucleon: fees are destroyed to compensate the entire network
...
// Nucleon: total coin age spent in transaction, in the unit of coin-days.
...
// Nucleon: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    // Nucleon: compute chain trust score
    pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

    // Nucleon: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // Nucleon: record proof-of-stake hash value
    ...
    // Nucleon: compute stake modifier
...
bool CBlock::CheckBlock() const
{
    ...
    // Nucleon: only the second transaction can be the optional coinstake
    for (int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // Nucleon: coinbase output should be empty if proof-of-stake block
...
bool CBlock::AcceptBlock()
{
    ...
    // Nucleon: check that the block satisfies synchronized checkpoint
    ...
    // Nucleon: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // Nucleon: verify hash target and signature of coinstake tx
    ...
    // Nucleon: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // Nucleon: getblocks may not obtain the ancestor block rejected
    ...
    // Nucleon: if responsible for sync-checkpoint send it
...
// Nucleon: entropy bit for stake modifier if chosen by modifier
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < (uint64)15000000 + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        ThreadSafeMessageBox(strMessage, "Nucleon", wxOK | wxICON_EXCLAMATION | wxMODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "Nucleon", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Nucleon: initialize synchronized checkpoint
        if (!Checkpoints::WriteSyncCheckpoint(hashGenesisBlock))
            return error("LoadBlockIndex() : failed to init sync checkpoint");

        // Nucleon: upgrade time set to zero if txdb initialized
        {
            CTxDB txdb;
    ...
    // Nucleon: if checkpoint master key changed must reset sync-checkpoint
    {
        CTxDB txdb;
        string strPubKey = "";
...
string GetWarnings(string strFor)
{
    ...
    // Nucleon: wallet lock warning for minting
    if (strMintWarning != "")
    {
        nPriority = 0;
    ...
    // Nucleon: if detected invalid checkpoint enter safe mode
    ...
    // Nucleon: if detected unmet upgrade requirement enter safe mode
    ...
    // Alerts
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
                ...
                if (nPriority > 1000)
                    strRPC = strStatusBar;  // Nucleon: safe mode for high alert
            }
        }
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // Nucleon: record my external IP reported by peer
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        // Nucleon: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // Nucleon: tell downloading node about the latest block if it's
...
CBlock* CreateNewBlock(CReserveKey& reservekey, CWallet* pwallet, bool fProofOfStake)
{
    ...
    // Nucleon: if coinstake available add coinstake tx
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // Nucleon: simplify transaction fee - allow free = false
...
void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        if (fProofOfStake)
        {
            // Nucleon: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
